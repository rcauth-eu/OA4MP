...
Some useful commands. Lst two list users and will show permissions for a single user on a machine.:
 ...
 Another note: The timestamp fields are given a default value of CURRENT_TIME
 since under MariaDB the default now is to change the value of any timestamp
 field on update, effectively rendering all of the "last modified". Setting to
 CURRENT_TIME unsets the other default of ON UPDATE CURRENT_TIMESTAMP
 */
...
GRANT All  ON client_approvals TO 'oa4mp-server'@'localhost';
GRANT ALL  ON clients TO 'oa4mp-server'@'localhost';
GRANT ALL  ON adminClients TO 'oa4mp-server'@'localhost';
GRANT ALL  ON transactions TO 'oa4mp-server'@'localhost';
GRANT ALL  ON permissions TO 'oa4mp-server'@'localhost';
